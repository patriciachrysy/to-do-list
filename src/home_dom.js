/*
  eslint-disable no-unused-vars, no-alert, no-undef,no-underscore-dangle, no-use-before-define
*/
...
import {
  getProjects, storeProject, updateProject, deleteProject,
} from './storage_module';
...
const manageForm = (projects) => {
  const valid = handleProjectForm('project-form');
  if (valid) {
    const project = new Project(valid);
...
const manageTaskForm = (position) => {
  const valid = handleTaskForm('task-form');
  if (valid) {
    const task = new Task(valid.title, valid.description, valid.dueDate, valid.priority, false);
    const project = getProjects()[position];
...
const manageUpdateTaskForm = (taskPos, projectPos) => {
  const valid = handleTaskForm('task-update-form');
  if (valid) {
    const project = getProjects()[projectPos];
    const task = project._tasks[taskPos];
...
const addTaskUpdateEvent = (position) => {
  ...
  if (project._tasks.length > 0) {
    ...
    updateButtons.forEach(button => {
      const taskPosition = button.parentElement.parentElement.getAttribute('data-task');
...
const manageUpdateProjectForm = (position) => {
  const valid = handleProjectForm('project-update-form');
  if (valid) {
    const project = getProjects()[position];
    project._name = valid;
    updateProject(project, position);
...
export {
  content, centerContent, buildMainPage, addProjectManagement, loadEventListeners,
};